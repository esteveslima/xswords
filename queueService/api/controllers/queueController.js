...
exports.getQueue = async (req, res, next) => {
  ...
  if(lockQueueNsp) return res.status(200).json({ status: true, path: process.env.QUEUE_WS_ROUTE, nameSpace: lockQueueNsp })
  ...
  if(nameSpace){    
    return res.status(200).json({ status: true, path: process.env.QUEUE_WS_ROUTE , nameSpace: nameSpace })
  }else{
    //tentar novamente?
